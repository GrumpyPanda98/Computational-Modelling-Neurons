...
for diameter in np.arange(2,5,0.50):
    ...
    diameter = diameter             # micrometers, diameter of the fiber
    ...
    def run_single_simulation(main_folder, title, t, waveform, time_step, time_stop, exit_t_shift, thresh_num_aps, conductivity, start_threshold, diameter):
        ...
        plot_simulation_results(folder_name, title, stimulation, fiber, waveform, stim_amp, ap, time)
        
        # Save fiber and stimulation objects to files
        with open(os.path.join(folder_name, "fiber.pkl"), "wb") as fiber_file:
            pickle.dump(fiber, fiber_file)
        
        with open(os.path.join(folder_name, "stimulation.pkl"), "wb") as stim_file:
            pickle.dump(stimulation, stim_file)
