...
def generate_waveforms(time_step, time_stop, folder_name, pre_stim=0):
...
# Modified run_single_simulation to accept precomputed_amp
def run_single_simulation(main_folder, title, t, waveform, time_step, time_stop, exit_t_shift, thresh_num_aps, conductivity, start_threshold, diameter, stim_multiplier, amp=None):
    ...
    # Use precomputed_amp if provided, else find threshold
    if amp is None:
        amp_found, ap_thresh = stimulation.find_threshold(fiber, exit_t_shift=exit_t_shift, thresh_num_aps=thresh_num_aps, silent=True)
        amp = amp_found
...
def run_simulation_parallel(main_folder, waveforms, time_step, time_stop, diameter, stim_multiplier, precomputed_amps=None):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=16) as executor:
        futures = [
            executor.submit(
                run_single_simulation, main_folder, title, t, waveform,
                time_step, time_stop, exit_t_shift, thresh_num_aps,
                conductivity, start_threshold, diameter, stim_multiplier,
                precomputed_amps.get(title) if precomputed_amps else None
            )
...
if __name__ == "__main__":
    tic=time.time()
    # Generate waveforms once outside the loop
    
    precomputed_amps = None

    for stim_multiplier in np.arange(1.18, 1.5, 0.02):
        tic = time.time()
        
        folder_name = fr"runs/stim_multiplier/{stim_multiplier.round(3)}"
        
        if not os.path.exists(folder_name):
            os.makedirs(folder_name)
        
        waveforms, waveform_configs = generate_waveforms(time_step, time_stop, folder_name, pre_stim)
        os.makedirs(folder_name, exist_ok=True)

        if stim_multiplier == 1.18:
            # First iteration: compute and save thresholds
            run_simulation_parallel(folder_name, waveforms, time_step, time_stop + pre_stim, diameter, stim_multiplier)
            # Load thresholds for future iterations
            with open(os.path.join(folder_name, "activation_thresholds.json"), 'r') as f:
                precomputed_amps = json.load(f)
        else:
            if precomputed_amps is None:
                # Attempt to load from first run
                first_folder = os.path.join("runs", "stim_multiplier", "1.0")
                threshold_file = os.path.join(first_folder, "activation_thresholds.json")
                if os.path.exists(threshold_file):
                    with open(threshold_file, 'r') as f:
                        precomputed_amps = json.load(f)
                else:
                    raise FileNotFoundError("Run with stim_multiplier=1.0 first to compute thresholds.")
            # Use precomputed thresholds
            run_simulation_parallel(folder_name, waveforms, time_step, time_stop + pre_stim, diameter, stim_multiplier, precomputed_amps)
